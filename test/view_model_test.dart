...
void main() {
  group('ViewModel', () {
    ...
    test('should call error handler for specific error type', () {
      ...
      uut.registerErrorHandler<ViewStatusError>(() => isErrorHandlerCalled = true);
    ...
    test('should handle error generically for unregistered error types', () {
      ...
      ConfigMVVM.initialize(onGenericError: (errorContext) {
    ...
    test('should safely execute a function handling ViewStatusError exceptions', () async {
      bool isOnErrorCalled = false;
      await uut.safeExecute(() => throw error, onError: (_) => isOnErrorCalled = true);
      expect(uut.state.viewStatus.isFailure, isTrue);
      expect((uut.state.viewStatus as ViewStatusFailure).error, equals(error));
      expect(isOnErrorCalled, isTrue);
    });

    test('should safely execute a function handling generic exceptions', () async {
      bool isOnErrorCalled = false;
      const exceptionMessage = 'Generic exception';
      await uut.safeExecute(() => throw Exception(exceptionMessage),
