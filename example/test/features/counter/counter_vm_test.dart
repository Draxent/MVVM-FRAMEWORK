...
void main() {
  group('CounterVM', () {
    late MockCounterRepository counterRepository;
    late CounterVM uut;
    const initValue = 7;
    ...
    setUp(() async {
      ...
      when(scaffoldMessenger.showSnackBar(any)).thenReturn(MockScaffoldFeatureController());
      when(uut.viewContext.scaffoldMessenger).thenReturn(MockScaffoldMessengerState());
    });
    ...
    test('should call logValue when value is divisible by 3', () async {
      await uut.listener(uut.state.copyWith(value: 8, viewStatus: viewStatusSuccess));
      verifyNever(counterRepository.logValue());
      await uut.listener(uut.state.copyWith(value: 9, viewStatus: viewStatusSuccess));
      verify(counterRepository.logValue()).called(1);
    });

    test('should handle exception when incrementWithException is called', () async {
      when(counterRepository.updateValue(any, true)).thenThrow(Exception('Test Exception'));
    ...
    test('should update state when didUpdateWidget is called with a new value', () async {
      const newValue = 10;
