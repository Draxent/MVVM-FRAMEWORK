...
void main() {
  group('RestaurantsVM', () {
    late RestaurantsVM uut;
    late MockRestaurantsRepository restaurantsRepository;
    late MockUserLocationRepository userLocationRepository;
    late MockFavouritesStorageService favouritesStorageService;
    ...
    test('should register error handler and listen to user location when initialized', () async {
      final streamController = StreamController<(LocationEntity, bool)>();
      when(userLocationRepository.watch()).thenAnswer((_) => streamController.stream);
      await uut.onInit();
      expect(uut.errorHandlers.containsKey(ViewStatusInvalidLocationError), true);
      verify(userLocationRepository.watch()).called(1);
      streamController.close();
    });

    test('should cancel stream subscription when closing the view model', () async {
    ...
    test('should set the restaurant as favourite', () async {
      ...
      verify(favouritesStorageService.setFavourite(restaurantId.id, value)).called(1);
    ...
    test('should handle view status error and pause location updates when fetch is failing',
        () async {
    ...
    test('should handle generic error and pause location updates when fetch is failing', () async {
      when(restaurantsRepository.fetch(locationZero)).thenThrow(Exception('error'));
      await uut.computeRestaurantsBasedOnUserLocation(locationZero, true);
      expect(uut.state.viewStatus, isA<ViewStatusFailure>());
      expect(uut.state.viewStatus, isA<ViewStatusFailure>());
      expect((uut.state.viewStatus as ViewStatusFailure).error.message, 'Exception: error');
      verify(userLocationRepository.pause()).called(1);
    });
    ...
    test('should set isInvalidLocation to true when onInvalidLocationError is called', () async {
      uut.onInvalidLocationError();
      expect(uut.state.isInvalidLocation, true);
    });

    test('should pause user location when isUserLocationPaused changes to true', () {
    ...
    test('should resume user location when isUserLocationPaused changes to false', () {
