...
class MockRestaurantsMVVM extends MVVM<RestaurantsModel, RestaurantsView, RestaurantsVM> {
...
void main() {
  group('RestaurantsPage', () {
    ...
    testWidgets('should reset location when location is invalid and button is pressed', (
      tester,
    ) async {
    ...
    testWidgets('should show a loading reset button when location is invalid and view is loading', (
      tester,
    ) async {
      model = model.copyWith(viewStatus: viewStatusLoading, isInvalidLocation: true);
      await tester.pumpWidget(uut);
      expect(find.text('User Location is invalid: '), findsOneWidget);
    ...
    testWidgets('should show the user location and forward walking image', (tester) async {
      model = model.copyWith(userLocation: const LocationEntity(latitude: 3, longitude: 5));
      await tester.pumpWidget(uut);
      expect(find.text('User Location (latitude: 3.0, longitude: 5.0)'), findsOneWidget);
      final gifWidget = tester.widget<Gif>(find.byType(Gif));
      expect(gifWidget.isFlippedHorizontally, false);
    });

    testWidgets('should show the user location and backward walking image', (tester) async {
    ...
    testWidgets('should show a loading text when view is loading', (tester) async {
      model = model.copyWith(restaurants: restaurants, viewStatus: viewStatusLoading);
      await tester.pumpWidget(uut);
