...
void main() {
  group('RestaurantsRepository', () {
    ...
    test('should return a list of restaurants sorted by distance', () async {
      ...
      final result = await uut.fetch(const LocationEntity(latitude: 5, longitude: 5));
    ...
    test('should throw ViewStatusError when random value is less than 0.15 and errorType is 0',
        () async {
    ...
    test(
      ...
      expect(() => uut.fetch(locationZero), throwsA(isA<ViewStatusNetworkError>()));
    });
    ...
    test(
      ...
      expect(() => uut.fetch(locationZero), throwsA(isA<ViewStatusInvalidLocationError>()));
    });
  });
}
