...
class RestaurantsPage extends MVVM<RestaurantsModel, RestaurantsView, RestaurantsVM> {
...
class RestaurantsView extends ViewWidget<RestaurantsVM> {
  ...
  Widget build(BuildContext context) {
    return const AppScaffold(title: 'Restaurants Feature', page: _RestaurantPageContent());
  }
}
    ...
    return Align(
      ...
      child: Row(
        ...
        children: [
          ...
          AppButton(label: 'Reset', isLoading: isLoading, onPressed: vm(context).resetLocation),
        ],
      ),
    );
  }
...
class _UserLocationView extends ViewWidget<RestaurantsVM> {
  ...
  @override
  Widget build(BuildContext context) {
    return const FittedBox(child: Row(children: [_UserLocationText(), _UserLocationGif()]));
  }
}
...
class _RestaurantListView extends ViewWidget<RestaurantsVM> {
  ...
  Widget build(BuildContext context) {
    ...
    return ListView.separated(
      separatorBuilder:
          (_, i) => const Padding(padding: EdgeInsets.only(left: 30), child: Divider()),
...
class _RestaurantItemView extends ViewWidget<RestaurantsVM> {
  ...
  @override
  Widget build(BuildContext context) {
    final isFavorite = select(context, (x) => x.state.favourites[item.id] ?? false);
    return ListTile(
      leading: ClipRRect(
        ...
        child: SizedBox(
          width: 56,
          height: 56,
          child: FittedBox(fit: BoxFit.fill, child: Image.network(item.imageUrl)),
        ),
      ),
