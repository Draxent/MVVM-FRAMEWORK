...
class RestaurantsVM extends ViewModel<RestaurantsModel> {
  ...
  Future<void> onInit() async {
    registerErrorHandler<ViewStatusInvalidLocationError>(onInvalidLocationError);
    streamSubscription = _userLocationRepository
        .watch()
        .listen((x) => computeRestaurantsBasedOnUserLocation(x.$1, x.$2));
  ...
  Future<void> computeRestaurantsBasedOnUserLocation(
      LocationEntity location, bool isUserDirectionForward) async {
    await safeExecute(
      () async {
  ...
  @visibleForTesting
  void onInvalidLocationError() => emit(state.copyWith(isInvalidLocation: true));
