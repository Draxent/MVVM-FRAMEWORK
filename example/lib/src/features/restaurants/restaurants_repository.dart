...
class RestaurantsRepository {
  RestaurantsRepository(@factoryParam Random? random) : _random = random ?? Random();
      ...
      switch (errorType) {
        ...
        case 2:
          throw ViewStatusInvalidLocationError(
              location: userPosition, message: 'Invalid $userPosition');
      }
    }
  }

  static double _calculateDistance(LocationEntity userLocation, LocationEntity restaurantLocation) {
    ...
    final lonDiff = (userLocation.longitude - restaurantLocation.longitude).abs();
    return latDiff + lonDiff;
  }

  final _restaurants = _generateRestaurants(
    10,
    [
  ...
  static List<RestaurantEntity> _generateRestaurants(int count, List<String> imageUrls) =>
      List.generate(
        count,
        (index) => RestaurantEntity(
          ...
          location: LocationEntity(latitude: index.toDouble(), longitude: index.toDouble()),
        ),
      );
}
